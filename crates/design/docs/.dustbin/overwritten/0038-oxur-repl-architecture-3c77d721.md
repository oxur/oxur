...
Purpose: Complete architectural specification for Oxur REPL system. All design decisions finalized based on Session 1 architecture review and comprehensive evcxr research (6+ years of git history + web documentation). Ready for implementation.
```
...
│  │  - Success: "EVCXR_EXECUTION_COMPLETE\n"                │  │
│  │  - Error: "EVCXR_RUNTIME_ERROR: <msg>\n"                │  │
│  └─────────────────────────────────────────────────────────┘  │
│                                                               │
└───────────────────────────────────────────────────────────────┘
                               ↓
...
Subprocess → REPL (via stdout):
  On success: "EVCXR_EXECUTION_COMPLETE\n"
  On error:   "EVCXR_RUNTIME_ERROR: panic message\n"
```
...
impl SubprocessExecutor {
    pub fn new() -> Result<Self> {
        // Spawn subprocess: oxur-subprocess binary
...
**Ownership:** Server (shared via Arc<ArtifactCache>)
...
These components run in the isolated subprocess.

#### Runtime

**Location:** `oxur-subprocess/src/main.rs` (binary in oxur-repl crate)
    ...
    for line in stdin.lock().lines() {
        ...
        if cmd.starts_with("LOAD_AND_RUN") {
            ...
            match runtime.load_and_execute(lib_path, fn_name) {
                Ok(_) => println!("EVCXR_EXECUTION_COMPLETE"),
                Err(e) => println!("EVCXR_RUNTIME_ERROR: {}", e),
            }
        }
...
│    On success: "EVCXR_EXECUTION_COMPLETE\n"             │
│    On error:   "EVCXR_RUNTIME_ERROR: <msg>\n"           │
│                                                         │
...
Sends error via stdout protocol:
  "EVCXR_RUNTIME_ERROR: Index out of bounds!\n"

Optional stack trace:
  "EVCXR_PANIC_LOCATION: lib.rs:47:9\n"

Step 13: SubprocessExecutor Handles Error
──────────────────────────────────────────
Reads "EVCXR_RUNTIME_ERROR" from stdout
...
Responses (Subprocess → Server via stdout):
  EVCXR_EXECUTION_COMPLETE\n                    (success)
  EVCXR_RUNTIME_ERROR: <error_message>\n        (error)
```
...
Server parses "EVCXR_EXECUTION_COMPLETE"
...
impl SubprocessExecutor {
        ...
        -> Result<ExecutionResult>
    {
        ...
        // 3. Parse response
        if line.starts_with("EVCXR_EXECUTION_COMPLETE") {
            Ok(ExecutionResult::Success { /* ... */ })
        } else if line.starts_with("EVCXR_RUNTIME_ERROR:") {
            let msg = line.strip_prefix("EVCXR_RUNTIME_ERROR: ")
                         .unwrap_or("");
...
### 10.3 Global Cache Directory ◄─ NEW
...
│   Protocol: "EVCXR_RUNTIME_ERROR: <msg>\n"     ◄─ NEW   │
...
impl SubprocessExecutor {
        ...
        -> Result<Response>
    {
        ...
        if response.starts_with("EVCXR_EXECUTION_COMPLETE") {
            Ok(Response::success())
        } else if response.starts_with("EVCXR_RUNTIME_ERROR:") {
            let msg = response
                .strip_prefix("EVCXR_RUNTIME_ERROR:")
                .unwrap()
                .trim();
...
4. **Decision 3a: IPC Mechanism - stdin/stdout** (SIMPLIFIED)
   ...
   - Text protocol: `LOAD_AND_RUN <path> <fn>` / `EVCXR_EXECUTION_COMPLETE`
   - Unix sockets deferred to v1.1+ (if needed)

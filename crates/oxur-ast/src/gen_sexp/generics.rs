...
impl Generator {
    ...
    pub fn generate_generics(&self, generics: &Generics) -> Result<SExp> {
        let params: Result<Vec<_>> = generics.params.iter().map(|p| self.generate_generic_param(p)).collect();
        let params = params?;

        Ok(typed_node("Generics", kwargs(vec![
    ...
    fn generate_generic_param(&self, param: &GenericParam) -> Result<SExp> {
        Ok(typed_node("GenericParam", kwargs(vec![
    ...
    fn generate_lifetime_param(&self, param: &LifetimeParam) -> Result<SExp> {
        ...
        Ok(typed_node("LifetimeParam", kwargs(vec![
            kwarg("ident", self.generate_ident(&param.ident)),
            kwarg("bounds", list(bounds)),
            kwarg("colon-span", match &param.colon_span {
    ...
    fn generate_type_param(&self, param: &TypeParam) -> Result<SExp> {
        let bounds: Result<Vec<_>> = param.bounds.iter().map(|b| self.generate_generic_bound(b)).collect();
        let bounds = bounds?;

        Ok(typed_node("TypeParam", kwargs(vec![
            kwarg("ident", self.generate_ident(&param.ident)),
            kwarg("bounds", list(bounds)),
            kwarg("default", match &param.default {
    ...
    fn generate_const_param(&self, param: &ConstParam) -> Result<SExp> {
        Ok(typed_node("ConstParam", kwargs(vec![
            kwarg("ident", self.generate_ident(&param.ident)),
    ...
    pub fn generate_where_clause(&self, where_clause: &WhereClause) -> Result<SExp> {
        let predicates: Result<Vec<_>> = where_clause.predicates.iter().map(|p| self.generate_where_predicate(p)).collect();
        let predicates = predicates?;

        Ok(typed_node("WhereClause", kwargs(vec![
            kwarg("has-where-token", if where_clause.has_where_token { sym("true") } else { sym("false") }),
    ...
    fn generate_where_predicate(&self, predicate: &WherePredicate) -> Result<SExp> {
        match predicate {
            ...
            WherePredicate::RegionPredicate(region) => {
                Ok(list(vec![sym("RegionPredicate"), self.generate_where_region_predicate(region)?]))
            }
    ...
    fn generate_where_bound_predicate(&self, predicate: &WhereBoundPredicate) -> Result<SExp> {
        let bounds: Result<Vec<_>> = predicate.bounds.iter().map(|b| self.generate_generic_bound(b)).collect();
        let bounds = bounds?;

        let bound_lifetimes: Result<Vec<_>> = predicate.bound_lifetimes.iter().map(|l| self.generate_lifetime_param(l)).collect();
        let bound_lifetimes = bound_lifetimes?;

        Ok(typed_node("WhereBoundPredicate", kwargs(vec![
    ...
    fn generate_where_region_predicate(&self, predicate: &WhereRegionPredicate) -> Result<SExp> {
        ...
        Ok(typed_node("WhereRegionPredicate", kwargs(vec![
            kwarg("span", self.generate_span(predicate.span)),
    ...
    fn generate_where_eq_predicate(&self, predicate: &WhereEqPredicate) -> Result<SExp> {
        Ok(typed_node("WhereEqPredicate", kwargs(vec![
            kwarg("span", self.generate_span(predicate.span)),

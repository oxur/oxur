...
fn test_execute_file_to_file() {
    ...
    // Verify the file was created (content will be empty for empty crate without header)
    let content = fs::read_to_string(output_file).unwrap();
    assert_eq!(content, "", "Empty crate should generate empty output without header");
}
